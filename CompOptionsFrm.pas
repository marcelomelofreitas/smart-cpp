...
begin
    LoadText;
    { TODO 4 -oSXKDZ -cTranslation : 汉化的问题处理掉 }
    SetOptions(true);
...
begin
    dlgresult := MessageDlg(
        Format('为便于 Smart-C++ 寻找, 建议您将编译器放入以下目录: ' + #13#10 + #13#10
        + '32位: %s' + #13#10 + '64位: %s'
        + #13#10 + #13#10 + '是否清空当前编译器配置文件列表?',
        [devDirs.Exec + 'MinGW32', devDirs.Exec + 'MinGW64']), mtConfirmation, [mbYes, mbNo, mbCancel], 0);
