...
const
    ...
    Associations: array[0..6, 0..3] of AnsiString = (
        ...
        ('h', 'C 头文件', '6', ''),
        ('hpp', 'C++ 头文件', '7', ''),
        { TODO 2 -oSXKDZ -cTranslation : 从Dev-C++转换到Smart-C++ }
        ('dev', 'Dev-C++ 工程文件', '3', 'xxx'),
        ('rc', '资源文件', '8', ''),
        ('template', 'Smart-C++ 模板文件', '1', ''));
...
begin
    ...
    try
        ...
        if reg.KeyExists('SmartCpp.' + Associations[Index, 0]) then begin
            reg.DeleteKey('.' + Associations[Index, 0]);
            reg.DeleteKey('SmartCpp.' + Associations[Index, 0]);
        end;
    finally
        reg.free;
    end;
...
begin
    ...
    for I := 0 to AssociationsCount - 1 do
        DdeOK[I] := (Associations[I, 3] <> '') or CheckDDEServer('SmartCpp.' + Associations[I, 0],
            'open',
            Uppercase(ChangeFileExt(ExtractFilename(Application.Exename), EmptyStr)));
