...
uses
    {$IFDEF WIN32}
    SynEditHighlighter, main, datamod, project, editor, devcfg;
{$ENDIF}
{$IFDEF LINUX}
QSynEditHighlighter, main, datamod, project, editor, devcfg;
{$ENDIF}
...
begin
    ...
    if SameStr('.dev', ExtractFileExt(FileName)) then begin
        ...
        if fFileDate = 0 then
            edTimestamp.Text := '(仅工程文件) -'
        else
            edTimestamp.Text := '(仅工程文件) ' + FormatDateTime(ShortDateFormat + ' hh:nn:ss', FileDateToDateTime(fFileDate));
